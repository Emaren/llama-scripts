...
# 📝 .envrc Setup (includes auto-freeze snapshot)
cat > .envrc <<EOF
...
# 🧠 For Starship prompt
export STARSHIP_VENV="$PROJECT_NAME-313"
export VIRTUAL_ENV_PROMPT="\$STARSHIP_VENV"

# 🧊 Auto pip freeze snapshot
timestamp=\$(date +%Y%m%d-%H%M)
mkdir -p venv-logs
freeze_log="venv-logs/venv-freeze-\$timestamp.log"
pip freeze > "\$freeze_log"
ln -sf "\$freeze_log" venv-logs/venv-freeze.log
cp "\$freeze_log" venv-logs/venv-freeze.log.prev
EOF
...
touch .gitignore
ADDED_ENTRIES=()
for LINE in '.envrc' '.direnv/' '.python-version' 'venv-logs/'; do
...
# 📦 Freeze snapshot → venv-logs/
mkdir -p venv-logs
FREEZE_LOG="venv-logs/venv-freeze-$(date +%Y%m%d-%H%M).log"
pip freeze > "$FREEZE_LOG"
echo "📦 Saved pip freeze → $FREEZE_LOG"
echo "📄 Top of freeze:"
head -n 5 "$FREEZE_LOG" | sed 's/^/   · /'

# 🔁 Freeze diff + symlink latest
if [[ -f venv-logs/venv-freeze.log ]]; then
  echo "🧬 Changes since last freeze:"
  diff venv-logs/venv-freeze.log "$FREEZE_LOG" | grep '^[<>]' || echo "No diff."
fi
ln -sf "$(basename "$FREEZE_LOG")" venv-logs/venv-freeze.log
cp "$FREEZE_LOG" venv-logs/venv-freeze.log.prev
